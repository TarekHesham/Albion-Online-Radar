...
class MobsHandler
{
    constructor(settings)
    {
        ...
        if (logEnemiesList) logEnemiesList.addEventListener("click", () => console.log(this.mobsList));
    }
    ...
    NewMobEvent(parameters) {
        ...
        const id = this.safeParseInt(parameters[0], null); // Entity ID
        const typeId = this.safeParseInt(parameters[1], null);
        const [posX, posY] = parameters[7] || [0, 0]; // Default position
        const exp = this.safeParseFloat(parameters[13], 0);
        const name = this.extractName(parameters, [32, 31]);
        const enchant = this.safeParseInt(parameters[33], 0);
        const rarity = this.safeParseInt(parameters[19], 1);
    
        // Route based on the existence of a name
        if (name) {
    ...
    // Utility function to safely parse integers with a default value
    safeParseInt(value, defaultValue = 0) {
        const parsed = parseInt(value);
        return isNaN(parsed) ? defaultValue : parsed;
    }
    
    // Utility function to safely parse floats with a default value
    safeParseFloat(value, defaultValue = 0) {
        const parsed = parseFloat(value);
        return isNaN(parsed) ? defaultValue : parsed;
    }
    
    // Utility function to extract a name from multiple possible parameter indices
    extractName(parameters, indices) {
        for (const index of indices) {
            if (parameters[index]) {
                return parameters[index];
            }
        }
        return null; // Fallback if no name is found
    }
    ...
    updateEnchantEvent(parameters)
    {
        ...
        console.log("Mobs Handler Same as animals :\n", 
            this.settings.harvestingLivingHide[`e${enemy.enchantmentLevel}`][enemy.tier-1]
        );
