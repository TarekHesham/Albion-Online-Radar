export class PlayersDrawing extends DrawingUtils 
{
    ...
    drawPlayer(ctx, players) {
        for (const playerOne of players)
        {
            // if (playerOne.nickname === undefined) continue;

            const point = this.transformPoint(playerOne.hX, playerOne.hY);
            this.drawFilledCircle(ctx, point.x, point.y, 15, "red");

            const textWidth = ctx.measureText(playerOne.currentHealth).width;
            this.drawText(point.x - textWidth /2, point.y + 24, playerOne.currentHealth, ctx, "12px", "yellow");
            this.drawText(point.x, point.y - 20, playerOne.name, ctx);
        }
    }
}
