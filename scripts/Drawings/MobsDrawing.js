export class MobsDrawing extends DrawingUtils
{
    ...
    invalidate(ctx, mobs, mists)
    {
        for (const mobOne of mobs)
        {
            ...
            {
                imageName = mobOne.name;
                imageFolder = "Mobs"; // Change folder to enemies
            }
            ...
            if (imageName !== undefined && imageFolder !== undefined) {
                this.DrawCustomImage(ctx, point.x, point.y, imageName, imageFolder, 50);
            } else {
                this.drawFilledCircle(ctx, point.x, point.y, 15, "#4169E1"); // Unmanaged ids
            }

            if (drawHp) {
                // Draw health bar
                const healthBarWidth = 40;
                this.drawFilledRectangle(ctx, point.x - healthBarWidth / 2, point.y + 25, healthBarWidth, 5, "#30d088");
    ...
    drawFilledRectangle(ctx, x, y, width, height, color)
    {
        ctx.fillStyle = color;
